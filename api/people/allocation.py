...
def allocate_shift(workers):
    ...
    for worker in workers:
        ...
        for slot in slots:
            ...
            if solver.Value(x[name, slot, "leader"]):
                task = "leader"

            elif solver.Value(x[name, slot, "register"]):
                task = "register"

            elif solver.Value(x[name, slot, "break"]):
                task = "break"

            schedule.append({
                "time": f"{slot // 60:02}:{slot % 60:02}",
                "task": task
            })
